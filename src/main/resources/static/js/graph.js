...
function add_nodes() {
    ...
    svg_nodes.append("image")
        ...
        .attr("xlink:href", function (d) {
            var img = d.image;
            if (img != undefined) {
                return "http://localhost:7476/rsks/path/images/" + d.image
            } else {
                return null;
            }
        })
...
function addToolTip(d) {
    ...
    if (d.image != undefined) {
        htmlStr = htmlStr + "<img src=\"http://localhost:7476/rsks/path/images/" + d.image + "\" height=\"100\" width=\"100\" />";
    }
...
function load() {
    d3.json("http://localhost:7476/rsks/hello/dataSource/type/1", function (error, root) { // 服务器加载节点圆形分区数据
        if (error) {
            return console.warn(error);
        }
        dataCirclePartition = root;
    });
    d3.json("http://localhost:7476/rsks/hello/dataSource/type/2", function (error, json) { // 服务器加载知识图谱数据
        if (error) {
            return console.warn(error);
        }
        console.log("初始加载：");
    ...
    // d3.json("http://localhost:7476/rsks/hello/dataSource/node/extend/99717", function (error, json) { // 服务器加载知识图谱数据
    //     if (error) {
    //         return console.warn(error);
    //     }
    //     console.log("初始加载：");
